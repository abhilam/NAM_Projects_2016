...
for k in xrange(5):#
    ...
    Fnme=Path+'Output_4Parms_NAM.csv'#'Output_4Parms_NAM.csv'#Output_4Parms_NAM_NY6NY7
    ...
    Data=open('/media/FantomHD/Abhi_Academics/Abhishes_Lamsal_lin/NAME_MAIZE/SimList_4Parms_ADAT_final.csv','rb');#SimList_4Parms_ADAT_final
    Data=csv.reader(Data,delimiter=',')
    next(Data,None)
    points_sob=np.ndarray(shape=(32400070,2))
    i=0
    for row in Data:
   ...
   # This is simulated part
    ...
    val1=val1[val1['Pop_No']<28].reset_index()
    points_sim=np.ndarray(shape=(len(val1),2))
    for i in xrange(len(val1)):
        points_sim[i][0]=val1['M'+b[k]][i]
        points_sim[i][1]=val1['M'+ba[k]][i]
    ...
    #Data_Inside.to_csv('Sobol_NY67_DataInside.csv')
    #Data_Outside.to_csv('Sobol_NY67_DataOutside.csv')
    #plt.figure(k)
    ...
    if type(ab)==shapely.geometry.multipolygon.MultiPolygon:
        print '******'
        for item in ab:
            plt.gca().add_patch(PolygonPatch(cascaded_union(item), alpha=0.5))
    else:plt.gca().add_patch(PolygonPatch(cascaded_union(triangles), alpha=0.5))
